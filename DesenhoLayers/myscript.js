	...
	*/
	...
	for (var i = 0; i < heights.length; i++) {
		...
		myLayer.click = function () {
			var info = { height: heights[i], color: colors[i], index: i };
			var currLayer = myLayer;
			return function (mouseData) {
				console.log("Layer clicado", currLayer);
				//aqui poderia fazer algo como abrir um modal com as informações do layer
				setTimeout(function () {
					document.getElementById("layer-container").innerHTML =
						"Layer clicado: " + info.index + "<br>" +
						"Altura: " + info.height + "<br>" +
						"Cor: " + info.color + "<br>";
				}, 0);
			};
		}();
...
var app = new PIXI.Application(
	width, height, { backgroundColor: 0xeeeeee }, { antialias: false }
);
